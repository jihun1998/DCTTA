...
# from finetune_SAM.models.sam_LoRa import LoRA_Sam


def parse_args():
    parser = argparse.ArgumentParser()
    
    parser.add_argument('--seed', type=int, default=5123)

    parser.add_argument('--model_dir', type=str, default='./sam_checkpoints/')
    ...
    parser.add_argument('--mode', type=str, default='sam_vanilla')
    parser.add_argument('--method', type=str, default='vanilla')

    parser.add_argument('--tta_lr', type=float, default=1e-6)
    parser.add_argument('--tta_num_iter', type=int, default=1)

    parser.add_argument('--lora', type=bool, default=False)
    # parser.add_argument('--if_update_encoder', type=bool, default=False)
    parser.add_argument('--if_encoder_lora_layer', type=bool, default=False)
    parser.add_argument('--encoder_lora_layer', type=list, default=[])
    ...
    # group_checkpoints.add_argument('--checkpoint', type=str, default='sam_vit_b_01ec64.pth')
    ...
    group_iou_thresh.add_argument('--target-iou', type=float, default=0.9,
...
def main():
    ...
    checkpoints_list, logs_path, logs_prefix, writer = get_checkpoints_list_and_logs_path(args, cfg)
    ...
    # Select experiment method (default: vanilla SAM)
    module_name = 'isegm.inference.evaluation_'+args.method
    module = importlib.import_module(module_name)
    evaluate_dataset = getattr(module, 'evaluate_dataset')
    ...
    for dataset_name in args.datasets.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            dataset_results = evaluate_dataset(dataset, predictor, args=args, pred_thr=args.thresh,
...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    exp_path = Path(args.exp_root + '/' + args.datasets + '/' + current_time_str + '_' + args.method + '_' + args.exp +'_' + str(args.tta_lr) +'_' + str(args.tta_num_iter))
    writer = SummaryWriter(log_dir=exp_path)

    os.system('cp -r ./isegm/inference/evaluation_'+args.method+'.py '+ str(exp_path)+'/evaluation.py')
    os.system('cp -r ./isegm/inference/predictors/'+args.mode+'.py '+ str(exp_path)+'/predictor.py')

    # write_data.py
    with open(str(exp_path)+'/args.txt', 'w') as f:
        f.write(str(args))

    return checkpoints_list, exp_path, logs_prefix, writer
