...
def evaluate_dataset(dataset, predictor, vis = True, vis_path = './experiments/vis_val/',**kwargs):
    ...
    start_time = time()
    iou_thrs = np.arange(0.8, min(0.95, 0.9) + 0.001, 0.05).tolist()
    
    from torch.utils.tensorboard import SummaryWriter

    writer = SummaryWriter(log_dir='/mnt/jihun5/colseg/experiments')
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        noc = len(sample_ious)
        noc_list, over_max_list = utils.compute_noc_metric(all_ious, iou_thrs=iou_thrs, max_clicks=20)
