...
class SamPredictor(object):
    ...
    def get_prediction_stateless(self, clicker, prev_mask=None, model = None,  mode='eval'):
        clicks_list = clicker.get_clicks()
        click = clicks_list[-1]
        last_y,last_x = click.coords[0],click.coords[1]
        self.last_y = last_y
        ...
        img = self.original_image
        mask, logit, logit_256 = self._get_prediction_stateless(img, [clicks_list], prev_mask=prev_mask, model=model)
        
        return mask, logit, logit_256

    def _get_prediction_stateless(self, img, clicks_lists, prev_mask=None, model = None):
        points_nd, click_label = self.get_points_nd(clicks_lists)
        points_nd[:, [0, 1]] = points_nd[:, [1, 0]]
        ...
        sam_output = self.net.predict_with_gradient_stateless(model,point_coords=points_nd.cpu().detach().numpy(),
                                                    point_labels=click_label.cpu().detach().numpy(),
                                                    mask_input=prev_mask,
                                                    multimask_output=flag_multimask, 
                                                    return_logits=True)
