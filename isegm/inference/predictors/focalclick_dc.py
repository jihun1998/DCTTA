...
class FocalPredictorDC(object):
    ...
    def get_prediction(self, clicker, prev_mask=None, prev_probs = None):
        ...
        if prev_probs is None:
            self.transforms[0].reset()
        
        self.transforms[0]._prev_probs =prev_probs
        ...
        # if torch.isnan(F.sigmoid(image_nd)).any() or torch.isinf(F.sigmoid(image_nd)).any():
        #     import pdb;pdb.set_trace()
        ...
        # if torch.isnan(F.sigmoid(pred_logits)).any() or torch.isinf(F.sigmoid(pred_logits)).any():
        #     import pdb;pdb.set_trace()
        ...
        # if torch.isnan(F.sigmoid(prediction)).any() or torch.isinf(F.sigmoid(prediction)).any():
        #     import pdb;pdb.set_trace()
        # if prev_mask.sum() > 0:
        #     import pdb;pdb.set_trace()
        ...
        #     coarse_mask_sig = torch.sigmoid(coarse_mask)
        #     # import pdb;pdb.set_trace()
        #     return coarse_mask_sig[0, 0]>0.49, coarse_mask[0, 0], coarse_mask_sig

        prediction  = torch.log( prediction/(1-prediction)  )
        # prediction  = torch.log( torch.clamp(prediction/(1-prediction), min =1e-7 ) )
        coarse_mask = prediction
        prev_mask = prev_mask
        clicks_list = clicker.get_clicks()
        image_full = self.original_image
        
        coarse_mask_np = coarse_mask.clone().detach().cpu().numpy()[0, 0] 
        prev_mask_np = prev_mask.clone().detach().cpu().numpy()[0, 0] 
        ...
        if ~torch.isnan(focus_pred).any():
            coarse_mask[:,:,y1:y2,x1:x2] =  focus_pred
        coarse_mask_sig = torch.sigmoid(coarse_mask)
        
        # if torch.isnan(coarse_mask_sig).any():
        #     import pdb;pdb.set_trace()
        # self.prev_prediction = coarse_mask_sig
        # self.transforms[0]._prev_probs = coarse_mask_sig.clone().detach().cpu().numpy()
        # self.transforms[0]._prev_probs = coarse_mask_sig.cpu().numpy()
        return coarse_mask_sig[0, 0]>0.49, coarse_mask[0, 0], coarse_mask_sig
    ...
    def _get_refine(self, coarse_mask, image, clicks, feature, focus_roi, focus_roi_in_global_roi):
        ...
        self.focus_coarse = torch.sigmoid(focus_coarse).clone().detach().cpu().numpy()[0, 0] * 255
        self.focus_refined = torch.sigmoid(focus_refined).clone().detach().cpu().numpy()[0, 0] * 255
