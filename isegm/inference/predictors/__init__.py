...
from .cdnet_dc import DiffisionPredictorDC
from .baseline import BaselinePredictor
from .focalclick import FocalPredictor
from .focalclick_dc import FocalPredictorDC
from .mfp import MFPPredictor
...
def get_predictor(net, brs_mode, device,
    ...
    if brs_mode == 'NoBRS':
    ...
    elif brs_mode == 'cdnet_dc':
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = DiffisionPredictorDC(net, device, zoom_in=zoom_in, with_flip=with_flip, infer_size =infer_size, **predictor_params_)
    ...
    elif brs_mode == 'focalclick_dc':
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = FocalPredictorDC(net, device, zoom_in=zoom_in, with_flip=with_flip, infer_size =infer_size, focus_crop_r= focus_crop_r, **predictor_params_)

    elif 'sam' in brs_mode:
        if predictor_params is not None:
            predictor_params_.update(predictor_params)

        module_name = "isegm.inference.predictors."+brs_mode
        module = importlib.import_module(module_name)
        SamPredictor = getattr(module, 'SamPredictor')
        predictor = SamPredictor(net, device, zoom_in=zoom_in, with_flip=with_flip, infer_size =infer_size, focus_crop_r= focus_crop_r, **predictor_params_)
        ...
        if predictor_params is not None:
            predictor_params_.update(predictor_params)
        predictor = MFPPredictor(net, device, zoom_in=zoom_in, with_flip=with_flip, **predictor_params_)
