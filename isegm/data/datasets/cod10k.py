...
class COD10kDataset(ISDataset):
    def __init__(self, dataset_path,
                 images_dir_name='Image', masks_dir_name='GT_Object',
                 **kwargs):
        ...
        self.dataset_samples = [x.name for x in sorted(self._images_path.glob('COD10K-CAM*.*'))]
        self._masks_paths = {x.stem: x for x in self._insts_path.glob('COD10K*.*')}
